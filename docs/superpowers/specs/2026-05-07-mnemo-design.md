...
### `@mnemo-mcp/server`
Wraps `@mnemo-mcp/core` as an MCP server. Tools exposed:
...
- **Unit**: `@mnemo-mcp/core` — capture/recall/forget/list/embed/index round-trips. Target 90% line coverage.
- **Integration**: `@mnemo-mcp/server` against a mock MCP client. `@mnemo-mcp/hooks (deferred)` against a recorded Claude Code hook payload corpus.
...
- **npm packages**: `@mnemo-mcp/core`, `@mnemo-mcp/server`, `@mnemo-mcp/hooks (deferred)`, `@mnemo-mcp/cli`. Single `mnemo` umbrella for `npx mnemo init`.
- **License**: MIT.
- **Brand**:
