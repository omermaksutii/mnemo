...
**Architecture:** Two-package npm workspace monorepo. `@omermaksutii/mnemo-core` is pure TS with no Claude Code dependency — the only thing that touches data. `@omermaksutii/mnemo-cli` is a thin commander-based wrapper that invokes core. Embeddings via `@huggingface/transformers` (ONNX Runtime + `all-MiniLM-L6-v2`, 384-dim, lazy-loaded). HNSW via `hnswlib-node`. Storage via `sql.js` (WASM SQLite). Test embedder is a deterministic hash-based fake so unit tests don't need the real model.
...
├── packages/
│   ├── core/
│   │   ├── package.json                  (@omermaksutii/mnemo-core)
│   │   ├── tsconfig.json
│   │   ├── src/
...
│   └── cli/
│       ├── package.json                  (@omermaksutii/mnemo-cli, bin: mnemo)
│       ├── tsconfig.json
│       ├── src/
...
## Task 2: Scaffold @omermaksutii/mnemo-core package
...
```bash
cd /Users/mac/Projects/mnemo && npm install && npm run build -w @omermaksutii/mnemo-core
```
...
```bash
git add -A && git commit -m "feat(core): scaffold @omermaksutii/mnemo-core package"
```
...
Run: `npm run build -w @omermaksutii/mnemo-core`
...
## Task 11: Scaffold @omermaksutii/mnemo-cli package
...
Expected: cli's deps and the local `@omermaksutii/mnemo-core` link.

- [ ] **Step 6: Commit**

```bash
git add -A && git commit -m "feat(cli): scaffold @omermaksutii/mnemo-cli package"
```
...
```bash
npx @omermaksutii/mnemo-cli remember --global "we use Vitest, never Jest"
npx @omermaksutii/mnemo-cli recall "which test framework do we use?"
```
