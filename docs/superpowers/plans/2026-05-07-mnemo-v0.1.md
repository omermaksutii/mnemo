...
│   └── cli/
│       ├── package.json                  (@mnemo/cli, bin: mnemo)
│       ├── tsconfig.json
│       ├── src/
...
## Task 2: Scaffold @mnemo/core package
...
```bash
cd /Users/mac/Projects/mnemo && npm install && npm run build -w @mnemo/core
```
...
```bash
git add -A && git commit -m "feat(core): scaffold @mnemo/core package"
```
...
Run: `npm run build -w @mnemo/core`
...
## Task 11: Scaffold @mnemo/cli package
...
Expected: cli's deps and the local `@mnemo/core` link.

- [ ] **Step 6: Commit**

```bash
git add -A && git commit -m "feat(cli): scaffold @mnemo/cli package"
```
