...
Claude Code session
  ...
  ├── @mnemo/mcp server          ← exposes recall/remember/forget/list/stats
  └── @mnemo/cli hooks           ← session-start, pre-task, post-edit auto-wiring
              │
              ▼
       @mnemo/core
...
```
@mnemo/core   — pure TS memory engine
@mnemo/mcp    — MCP server (drop into Claude Code)
@mnemo/cli    — terminal commands + hook handlers + init
```
